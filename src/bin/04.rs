...
use std::{num::ParseIntError, collections::{HashSet, VecDeque}};
...
fn total_cards(cards: &str) -> u32 {
    cards.lines().map(count_corresponding_numbers).fold((0, VecDeque::with_capacity(25)), |(total_card, mut next_copies), n| {
...
fn count_corresponding_numbers(card: &str) -> u16 {
    let i = card.find(':').expect("Invalid input, cannot find the start of card");
    let card = &card[i..];
    let i = card.find('|').expect("Invalid input, cannot find the separator of winning numbers & numbers we have");
    let winning_numbers = &card[2..i-1];
    let numbers_you_have = &card[i+2..];
    
    let winning_numbers: Result<HashSet<u16>, ParseIntError> = parse_numbers(winning_numbers).collect();
    let winning_numbers = winning_numbers.expect("Invalid input, cannot parse winning numbers");

    parse_numbers(numbers_you_have).filter(|n| {
        let n = n.as_ref().expect("Invalid input, cannot parse numbers we have");
